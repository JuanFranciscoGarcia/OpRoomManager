"""
Created on Thu Mar  5 07:30:48 2026
...
def greedy_routes_fixed(demands, capacity, distances, clientes):
    depot = DEPOSITO
    depot_aux = 'DepósitoAUX'

    # Crear una copia local de las estaciones restantes excluyendo depósito y depósito auxiliar
    remaining_stations = [cliente for cliente in clientes if cliente not in ["Depósito", "DepósitoAUX"]]
    routes = []
    ...
    while remaining_stations:
        ...
        while remaining_stations:
            # Filtrar estaciones factibles según la capacidad restante
            feasible_stations = [
                station for station in remaining_stations
                if current_capacity + demanda.loc[demands["Unnamed: 0"] == station, "Demanda"].values[0] <= capacity
            ]
            ...
            next_station = min(
                feasible_stations,
                key=lambda x: distances.at[current_station, x]
            )
            current_route.append(next_station)
            current_capacity += demanda.loc[demands["Unnamed: 0"] == next_station, "Demanda"].values[0]
            remaining_stations.remove(next_station)
            current_station = next_station

        current_route.append(depot_aux)  # Terminar la ruta en 'DepósitoAUX'
        routes.append(current_route)

    return routes

rutas = greedy_routes_fixed(demanda, CAPACIDAD_CAMION, matriz_distancias, clientes)
print(rutas)
